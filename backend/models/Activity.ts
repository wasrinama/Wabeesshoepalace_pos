...
// Type definitions for activity categories
export type ActivityCategory = 'auth' | 'user_management' | 'inventory' | 'sales' | 'finance' | 'system' | 'security';

// Type definitions for activity severity
...
interface IActivityModel extends Model<IActivity> {
  createLog(logData: Partial<IActivity>): Promise<IActivity>;
}
...
activitySchema.statics.createLog = async function(logData: Partial<IActivity>): Promise<IActivity> {
  try {
...
activitySchema.methods.getActionDescription = function(this: IActivity): string {
...
const Activity: IActivityModel = mongoose.model<IActivity, IActivityModel>('Activity', activitySchema);
