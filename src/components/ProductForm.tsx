...
// Enhanced interfaces for the ProductForm component
...
interface ProductFormProps {
  ...
  onCancel: () => void;
  onUpdateSuppliers: (suppliers: ISupplier[]) => void;
  onUpdateCategories: (categories: ICategory[]) => void;
}
...
const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const [currentSuppliers, setCurrentSuppliers] = useState<ISupplier[]>([]);
  const commonSizes: string[] = ['5', '6', '7', '8', '9', '10', '11', '12'];
  const commonColors: string[] = ['Black', 'White', 'Brown', 'Red', 'Blue', 'Navy', 'Gray'];

  // Safety function to ensure data is properly structured
  const safeCategories = (cats: ICategory[] | undefined): ICategory[] => {
    if (!Array.isArray(cats)) return [];
    return cats.filter(cat => 
      cat && 
      typeof cat === 'object' && 
      (cat._id || (cat as any).id) && 
      cat.name && 
      typeof cat.name === 'string'
    );
  };

  const safeSuppliers = (sups: ISupplier[] | undefined): ISupplier[] => {
    if (!Array.isArray(sups)) return [];
    return sups.filter(sup => 
      sup && 
      typeof sup === 'object' && 
      ((sup as any).id || sup._id) && 
      sup.name && 
  ...
  useEffect(() => {
    if (product) {
      ...
      const sizeQuantities: SizeQuantity[] = (product as any).sizes ? 
        (product as any).sizes.map((size: string) => ({ 
          size, 
          quantity: Math.floor((product as any).stock / (product as any).sizes.length) || 1 
        })) : 
        [];
      ...
      setFormData({ 
        ...product as any, 
        category: typeof (product as any).category === 'object' ? 
          (product as any).category?._id || (product as any).category?.id : 
          (product as any).category || '',
        supplier: typeof (product as any).supplier === 'object' ? 
          (product as any).supplier?.name : 
          (product as any).supplier || '',
        brand: typeof (product as any).brand === 'object' ? 
          (product as any).brand?.name : 
          (product as any).brand || '',
        name: String(product.name || ''),
        price: String(product.price || (product as any).sellingPrice || ''),
        costPrice: String(product.costPrice || ''),
        minStock: String((product as any).minStock || (product as any).reorderLevel || ''),
  ...
  const handleSizeQuantityChange = (size: string, quantity: string): void => {
    setFormData(prev => ({
      ...prev,
  ...
  const calculateTotalStock = (): void => {
    setTimeout(() => {
      setFormData(prev => ({
        ...prev,
  ...
  const handleAddSupplier = (): void => {
    if (newSupplier.trim() && !currentSuppliers.some(s => s.name === newSupplier.trim())) {
      const newSupplierObj: ISupplier = {
        _id: Date.now().toString(),
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                required
              />
              {errors.name && <span className="text-red-500 text-sm mt-1">{getErrorMessage(errors.name)}</span>}
            </div>
            ...
            <div>
              ...
              <input
                ...
                required
              />
              {errors.brand && <span className="text-red-500 text-sm mt-1">{getErrorMessage(errors.brand)}</span>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              />
              {errors.sku && <span className="text-red-500 text-sm mt-1">{getErrorMessage(errors.sku)}</span>}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              {errors.category && <span className="text-red-500 text-sm mt-1">{getErrorMessage(errors.category)}</span>}
            </div>
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {currentSuppliers.map(supplier => (
                    <option key={(supplier as any).id || supplier._id} value={supplier.name}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                required
              />
              {errors.price && <span className="text-red-500 text-sm mt-1">{getErrorMessage(errors.price)}</span>}
            </div>
            ...
            <div>
              ...
              <input
                ...
                required
              />
              {errors.costPrice && <span className="text-red-500 text-sm mt-1">{getErrorMessage(errors.costPrice)}</span>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              {errors.minStock && <span className="text-red-500 text-sm mt-1">{getErrorMessage(errors.minStock)}</span>}
            </div>
          </div>
          ...
          <div>
            ...
            {errors.sizeQuantities && <span className="text-red-500 text-sm mt-1">{getErrorMessage(errors.sizeQuantities)}</span>}
          </div>
          ...
          <div>
            ...
            {errors.colors && <span className="text-red-500 text-sm mt-1">{getErrorMessage(errors.colors)}</span>}
          </div>
